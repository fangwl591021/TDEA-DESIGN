(() => {
  ...
  const api = async (path, options = {}) => {
    ...
    if (!response.ok) throw new Error(payload.error || payload.message || `操作失敗（HTTP ${response.status}）`);
    return payload;
  };
  ...
  async function ensureLiffReady() {
    ...
    try { if (typeof liff.getContext === 'function' && liff.getContext()) return; } catch (_) {}
    if (!scannerLiffInitPromise) {
      ...
      })().catch((error) => { scannerLiffInitPromise = null; throw error; });
    }
    await scannerLiffInitPromise;
  }

  function showMember(data) {
    const member = data.member || {}, wallet = data.wallet || {}, caps = data.capabilities || { canCreditPoints:true, canDebitPoints:true };
    const layer = modal(`<div style="display:flex;justify-content:space-between;align-items:center"><div><small>TDEA 點數收銀台</small><h2 style="margin:4px 0;color:#b95121">會員點數操作</h2></div><button data-point-op-close style="border:0;border-radius:50%;width:40px;height:40px;background:#b95121;color:white;font-size:24px">×</button></div><div style="margin:16px 0;padding:14px;border-radius:14px;background:#fff6f0"><strong>${esc(member.displayName || '會員')}</strong><div>${esc(member.memberNumber || '')}</div></div><div>目前點數</div><div style="font-size:34px;font-weight:900;color:#b95121">${Number(wallet.balance || 0).toLocaleString('zh-TW')} 點</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:16px">${caps.canCreditPoints !== false ? '<button data-point-action="grant" style="min-height:48px;border:0;border-radius:14px;font-weight:800;background:#e8f6ee;color:#137347">＋ 贈點</button>' : ''}${caps.canDebitPoints !== false ? '<button data-point-action="deduct" style="min-height:48px;border:0;border-radius:14px;font-weight:800;background:#fff0ec;color:#b43b22">－ 扣點</button>' : ''}</div>`);
  ...
  async function startScan() {
    ...
    scanStarting = true;
    try {
      // 臨時全面開放模式：前端不再呼叫 /v1/point-operator/access，也不做白名單判斷。
      modal('<h2 style="color:#b95121">TDEA 點數掃碼</h2><p>正在開啟掃描器…</p>');
  ...
  function bindAvatar() {
    ...
    avatar.onclick = (event) => { event.preventDefault(); event.stopPropagation(); startScan(); return false; };
  }
