...
function renderPersonalCalendarView() {
  ...
  layout(`${reminderHtml}<section class="personal-calendar-labels"><header><div><small>顯示篩選</small><h2>我的行事曆標籤</h2></div><button id="calendarAddLabel" type="button">＋ 自訂標籤</button></header><p class="muted">點擊標籤可顯示／隱藏行事曆內容。</p><div>${labelHtml}</div></section><section class="personal-calendar-card"><header class="personal-calendar-toolbar"><button id="calendarPrev" type="button">‹</button><div><small>PERSONAL CALENDAR</small><h2>${year} 年 ${month} 月</h2></div><button id="calendarNext" type="button">›</button></header><div class="personal-calendar-week"><span>日</span><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span></div><div class="personal-calendar-grid">${cells.join("")}</div><div class="personal-calendar-sync-note"><span>●</span> 本月顯示 ${monthSessions.length} 項個人、活動與生日行程</div></section><section class="personal-calendar-agenda"><header><div><small>當日行程</small><h2>${esc(selectedLabel)}</h2></div><div><button id="calendarToday" type="button">今天</button><button id="calendarAddEvent" type="button">＋ 行程</button></div></header>${eventHtml}</section>`);
  document.querySelectorAll("[data-calendar-date]").forEach((button)=>button.onclick=()=>{state.calendarSelectedDate=button.dataset.calendarDate||"";renderPersonalCalendarView();requestAnimationFrame(()=>document.querySelector(".personal-calendar-agenda")?.scrollIntoView({behavior:"smooth",block:"start"}));});
  ...
  layout(`<section class="ak-dashboard">
    <header class="ak-member-summary">
      <div class="ak-member-brand-row">
        <strong class="ak-wordmark">TDEA</strong>
...
async function daily(targetSelector = "") {
  ...
  if (state.dailyPanel !== "checkin") {
    try {
      ...
      const activityMarkup = activities.length ? `<section class="tdea-activity-carousel" aria-label="TDEA 活動報名">${activities.map((activity) => `<article class="card tdea-activity-card"><div class="tdea-activity-poster">${activity.imageUrl ? `<img src="${esc(activity.imageUrl)}" alt="${esc(activity.title)}" loading="lazy">` : `<span>${esc(activity.title || "TDEA 活動")}</span>`}</div>${activity.registrationUrl ? `<div class="tdea-activity-actions"><a class="btn alt" href="${esc(activity.registrationUrl)}" target="_blank" rel="noopener noreferrer">詳細說明</a><a class="btn" href="${esc(activity.registrationUrl)}" target="_blank" rel="noopener noreferrer">點我報名</a></div>` : ""}</article>`).join("")}</section>` : `<div class="card muted">目前沒有公開活動。</div>`;
