...
test("TDEA admin whitelist is checked server-side without creating an activity", () => {
  ...
  assert.match(helper, /https:\/\/tdea\.internal\/api\/activities/);
  assert.match(helper, /"x-admin-email": email/);
  assert.match(helper, /JSON\.stringify\(\{ name: "" \}\)/);
